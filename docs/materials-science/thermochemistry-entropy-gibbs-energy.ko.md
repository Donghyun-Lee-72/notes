...
밖의 모든 것이며, 계와 주위를 합쳐 열역학적 우주라고 합니다. 열량계 모형에서
용액 반응을 계로 정하고 용액, 컵, 온도계, 외부 실내를 주위의 일부로 볼 수
있습니다
...
부호와 단위가 서로 맞습니다. 열량계가 얻은 양의 열은 계가 잃은 음의 열에
...
여기서 `p`는 계의 압력, `V`는 부피이고, 압력–부피 일만 일어날 때
`q_p = ΔH`입니다. 이 등식들은 조건이 붙은 측정 관계이지 열, 내부 에너지,
엔탈피를 서로 바꿔 부르는 표현이 아닙니다
...
통계적 설명에서는 `S = k_B ln W`입니다. 여기서 `k_B`는 볼츠만 상수,
`ln`은 자연로그, `W`는 모형의 제약 아래 접근 가능한 미시상태 수입니다.
따라서 엔트로피는 물질과 에너지가 접근 가능한 상태에 분포할 수 있는 방식과
관련됩니다. 엔트로피를 단지 “무질서도”라고 부르면 모형, 제약, 정량적 정의를
숨기게 됩니다. 한 계의 엔트로피는 감소할 수 있습니다. 열역학 제2법칙은
완전한 고립 전체에 적용됩니다.
...
그 밖의 주위와 고립되어 있다고 합시다. 유한한 온도 차이를 가로지르는 실제
열전달은 비가역적이지만, 각 열저장고의 두 끝 상태를 잇는 가역 경로를 사용해
엔트로피 변화를 계산할 수 있습니다.
...
강제하지 않을 때, **실제** 깁스 에너지 변화의 부호는 열역학적 방향을
나타냅니다.

| 명시된 조건에서의 실제 변화 | 해석 |
| --- | --- |
| `ΔG < 0` | 정방향 변화가 열역학적으로 유리함 |
| `ΔG = 0` | 그 변화에 대해 평형임 |
| `ΔG > 0` | 정방향은 유리하지 않고 역방향이 유리함 |

이 판정 기준은 위 제약 아래에서 열역학 제2법칙으로부터 나옵니다
([OpenStax, §16.4](https://openstax.org/books/chemistry-2e/pages/16-4-free-energy)).
임의로 변하는 온도나 압력에 적용하는 규칙이 아니며, 이것만으로 개방되거나
외부에서 구동되거나 비평형 정상상태인 계를 설명할 수 없습니다.

**깁스 에너지 점검.** `298 K`의 일정 압력에서 일어나는 가상 과정의 값이
`ΔH = +18.0 kJ mol^-1`, `ΔS = +75.0 J mol^-1 K^-1`이라고 합시다.
먼저 엔트로피 단위를 바꿉니다.
...
위 판정 기준에 따르면 정방향 과정은 `298 K`에서 열역학적으로 유리합니다.
`ΔH`와 `ΔS`가 온도에 따라 거의 일정하다면 부호가 바뀌는 온도는
`T = ΔH/ΔS = 18.0/0.0750 = 240 K` 부근입니다. 이 교차 온도는
근사값입니다. 열용량, 상변화, 조성 때문에 `ΔH`와 `ΔS`가 온도에 따라
달라질 수 있습니다
([OpenStax, §16.4](https://openstax.org/books/chemistry-2e/pages/16-4-free-energy)).

흔히 쓰는 부호 표에도 같은 근사가 들어 있습니다.

| `ΔH` | `ΔS` | `ΔG = ΔH - TΔS`로 추론한 방향 |
| --- | --- | --- |
...
활동도로 정의한 무차원 반응 지수입니다. 따라서 `Δ_rG° < 0`이라고 해서 모든
조성에서 정반응이 유리한 것은 아닙니다. 이후 평형 문서에서 `Q`, 활동도, 평형
상수를 자세히 다룹니다
([OpenStax, §16.4](https://openstax.org/books/chemistry-2e/pages/16-4-free-energy)).
...
빠르게, 어떤 경로로 진행되는지를 묻습니다. 음의 `ΔG`는 시간을 알려 주지
않습니다. 이용 가능한 경로의 활성화 장벽이 크면 관찰 시간 동안 변화를
검출하지 못할 만큼 느릴 수 있습니다. 촉매는 경로와 속도를 바꿀 수 있지만
같은 조건에서 반응의 초기·최종 `ΔG`나 평형 위치는 바꾸지 않습니다
...
반응물을 없애도록 강제하는 대신 `Δ_rG`가 평형에서 0에 가까워집니다.
...
4. **양의 종류:** 값이 `ΔU`, `q`, `ΔH`, `ΔS`, `ΔG` 또는 표준 상태
   양인지, 아니면 종류를 밝히지 않은 “에너지”인지 구분했는가?
...
한다는 생각, 음의 `ΔG`가 빠르거나 완결되는 반응을 보장한다는 생각,
`ΔG°`가 모든 조성을 나타낸다는 생각이 있습니다. 각각 결론에 필요한 조건을
빠뜨린 것입니다.
