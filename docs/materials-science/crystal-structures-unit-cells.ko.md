...
집단과 이를 3차원 공간에 재현하는 병진 이동으로 기술한다. 이런 반복
표현을 쓰면 거대한 고체를 다루기 쉬운 기하학적 모델로 나타낼 수 있다.
또한 원자 수준의 기하와 방향을 밀도, 변형, 확산, 전기적 응답 등의 성질과
연결하는 데 도움이 된다. 이 모델이 실제 시편의 반복 자리마다 결함이
없음을 뜻하지는 않는다
...
서로 관련된 네 용어는 각각 다른 질문에 답한다.

- **격자(lattice)**는 서로 동등한 점으로 이루어진 무한한 수학적 배열이다.
  병진 주기성을 나타낼 뿐 그 자체에는 화학적 정체성이 없다.
- **기저(basis)** 또는 **모티프(motif)**는 모든 격자점에 같은 방향으로
  붙이는 원자, 이온, 분자 또는 입자 집단이다.
- **결정 구조**는 모든 격자점에 기저를 놓아 만든다. 즉,
  `결정 구조 = 격자 + 기저`이다.
- **단위 격자(unit cell)**는 동일 평면에 놓이지 않는 세 병진 벡터로
  정해지는 평행육면체이다. 이 벡터들의 정수배만큼 이동시키면 빈틈이나
  겹침 없이 공간을 채운다.

이 구분은 흔한 오해를 막아 준다. 그려진 셀 중앙에 점이 있다고 해서 결정
구조가 자동으로 체심 Bravais 격자가 되는 것은 아니다. 화학적 기저도 함께
고려해야 한다. IUCr는 격자를 원자 부분 구조가 아니라 주기성의 수학적
기술로 명시하며, 단위 격자를 세 결정학적 기저 벡터로 정의한다
([IUCr, “Substructure”](https://dictionary.iucr.org/Substructure);
[IUCr, “Unit cell”](https://dictionary.iucr.org/Unit_cell)).

셀 벡터의 길이는 `a`, `b`, `c`이다. 축간각은
`α = ∠(b,c)`, `β = ∠(a,c)`, `γ = ∠(a,b)`이다. 이 여섯
...
격자점을 포함한다. 병진으로 격자를 재현할 수 있는 최소 부피의 셀이다.
그러나 격자 대칭을 가장 알아보기 쉽게 나타내지는 않을 수 있다.

**관습 셀(conventional cell)**은 대칭과 기하를 쉽게 보이도록 선택한다.
비원시 셀일 수 있으며 격자점을 둘 이상 포함할 수 있다. 익숙한 체심 및
면심 입방 관습 셀은 각각 격자점 2개와 4개를 포함한다. 벡터와 내용물을
일관되게 명시하면 원시 셀과 관습 셀 모두 같은 무한 격자를 나타낸다.
따라서 “단위 격자”가 언제나 “가장 작은 셀”을 뜻하지는 않는다
...
셀 경계는 원자를 물리적으로 자르는 면이 아니라 계산을 위한 면이다. 입방
관습 셀에서 완전히 내부에 있는 자리는 `1`, 면 중심은 `1/2`, 모서리
중심은 `1/4`, 꼭짓점은 `1/8`만 기여한다. 따라서

`N = N_i + N_f/2 + N_e/4 + N_c/8`

이다. 여기서 `N`은 셀에 귀속되는 유효 입자 수이며, `N_i`, `N_f`,
`N_e`, `N_c`는 각각 내부, 면, 모서리, 꼭짓점에 그려진 입자 수이다.
공유 분율은 셀 기하에 따라 달라진다. 관습적인 육각기둥의 꼭짓점은 이런
기둥 여섯 개가 공유하므로 `1/6`이지 `1/8`이 아니다.

여러 화학종이 있는 구조에서는 화학종별로 따로 센다. 전체 원자 수가
맞더라도 화학식은 틀릴 수 있다.

## SC, BCC, FCC, HCP를 기하 모델로 비교하기

다음 결과는 동일한 단단한 구가 명시한 방향에서 서로 접한다고 가정한다.
`R`은 구 반지름, `a`는 입방 셀 모서리 길이이다. **배위수**는 같은
최단 이웃 거리에서 만나는 최근접 이웃 수이다. **원자 충진율(APF)**은

`APF = (셀에 귀속되는 단단한 구의 부피)/(셀 부피)`

이며 무차원이다. APF는 측정된 기공률이 아니며 전자 밀도를 세는 값도
아니다. 동일 구 모델의 기하학적 결과이다
...
대수 계산은 유용한 검산이 된다. 예를 들어 FCC에서는
`a = 2√2R`이므로 셀 부피는 `a^3 = 16√2 R^3`이다.
구 네 개의 부피는 `4(4πR^3/3) = 16πR^3/3`이고 그 비는
`π/(3√2) ≈ 0.7405`이다.

### 육방 최밀 구조

**육방 최밀(HCP)** 구조는 최밀면을 `ABAB…` 순서로 쌓는다. FCC는
대안적인 `ABCABC…` 최밀 적층으로 볼 수 있다. 이상적인 동일 구
충진에서 두 구조 모두 배위수는 12이고 APF는
`π/(3√2) ≈ 0.74`이지만, 병진 대칭과 적층 반복은 다르다
([LibreTexts, §12.2](https://chem.libretexts.org/Bookshelves/General_Chemistry/Book%253A_General_Chemistry%253A_Principles_Patterns_and_Applications_%28Averill%29/12%253A_Solids/12.02%253A_The_Arrangement_of_Atoms_in_Crystalline_Solids);
Callister and Rethwisch, 2015, Ch. 3, §§3.4 and 3.16, pp. 53–56, 89–90).

관습적인 육각기둥 표현에는
`12(1/6) + 2(1/2) + 3 = 6`개의 원자가 귀속된다. 이상적인 접촉
구에 대해 `a = 2R`이고

`c/a = √(8/3) ≈ 1.633`

이다. 여기서 `c`는 기둥 높이이다. 실제 HCP 결정의 `c/a` 비는 이 값과
다를 수 있으므로 “HCP”가 이상적 구 기하를 보장하지는 않는다. 원시 HCP
셀에는 원자 두 개가 들어가며, 원자 여섯 개짜리 기둥은 편리한 비원시
셀이다
...
화합물에서는 `nM`을 `Z M_formula`로 바꾼다. `Z`는 셀당 화학식 단위
수이고 `M_formula`는 화학식 단위 하나의 몰 질량이다. 분자와 부피는
반드시 같은 셀을 가리켜야 한다. 이 식은 셀당 질량을 셀 부피로 나눈
것이다
...
생각하자. 이는 실제 물질의 데이터가 아니라 교육용 입력값이다.
...
이다. 몰 단위와 10의 거듭제곱은 올바르게 소거된다. 역검산하면
`ρV_cell = 3.99 × 10^-22 g`이고, 이는 반올림 오차 안에서 모델 원자
네 개의 질량 `4M/N_A`와 같다. 이 계산은 완전한 셀의 예측이다. 공공,
...
삼방정계, 육방정계, 입방정계이다. 이들은 3차원 결정 대칭을 분류하며,
관습 셀은 `a`, `b`, `c`, `α`, `β`, `γ` 사이의 제약으로
기술한다. 허용되는 대칭을 원시 및 허용 중심화 방식과 조합하면 3차원
**Bravais 격자 유형** 14개가 나온다. Bravais 격자는 병진 대칭을
분류하지, 화학적 기저나 완전한 결정 구조를 분류하는 것은 아니다
([IUCr educational pamphlet 2](https://www.iucr.org/education/pamphlets/2/full-text)).

이 수준에서 중요한 것은 다음 계층이다.

`결정학적 대칭 + 관습 셀의 계량 제약 → 결정계; 병진과 중심화 → Bravais 격자; 격자 + 기저 → 결정 구조`

SC, BCC, FCC는 세 입방 Bravais 격자 유형이다. HCP는 원시 육방
Bravais 격자에 원자 두 개짜리 기저를 놓은 결정 구조이며 별도의 Bravais
격자가 아니다. 점군, 공간군, 좌표축 설정 관례의 자세한 내용은 이후의
결정학에서 다룬다.

## 구조 때문에 방향이 중요해진다

시편 방향에 따라 측정값이 달라지는 성질은 **이방성**이고, 달라지지
않으면 **등방성**이다. 입방 결정이라도 모든 성질에 대해 자동으로
등방성이 되는 것은 아니다. 대칭은 어느 방향의 값이 같아야 하는지를
제약하고, 그 성질의 텐서 특성이 나머지 가능성을 정한다.

따라서 단결정은 탄성, 광학, 전기, 자기 또는 수송 응답에서 방향 의존성을
보일 수 있다. 무작위 방향의 결정립이 많은 다결정은 많은 결정립을 포함하는
척도에서 근사적으로 등방성처럼 보일 수 있다. 그러나 우선 배향
(**집합조직**), 결정립 모양, 상 또는 공정 때문에 거시적 이방성이 남을 수
있다. 무작위 배향은 시험할 가정이지 다결정이라는 사실에서 자동으로
나오는 결과가 아니다
...
있다는 뜻이다. 이러한 구조의 다중성이 화합물이 아니라 원소 물질에서
나타날 때 **동소체성**이라고 한다. 안정한 형태는 온도와 압력에 따라
달라질 수 있고, 변환은 속도론적으로 지연될 수 있다. 구조 변화는 전체
조성을 바꾸지 않고도 밀도와 다른 성질을 바꿀 수 있다
...
- 완전한 주기성은 표면, 공공, 침입형 원자, 치환, 전위, 결정립계, 무질서,
  열운동을 생략한다.
- 단단한 구는 날카로운 반지름과 쌍별 접촉을 가정한다. 실제 전자 밀도는
  연속적이고 결합은 방향성을 띨 수 있으며, 유효 반지름은 화학적 환경에
  따라 달라진다.
- 배위에는 명시된 이웃 판정 규칙이 필요하다. 왜곡되었거나 여러 화학종이
  있는 구조에서는 거리 절단값이 모호할 수 있다.
- APF는 이상 셀 안의 동일한 단단한 구 부피를 비교한다. 벌크 기공률이나
  보편적인 안정성 척도가 아니다.
- 셀 그림은 유일하지 않다. 원시 셀과 관습 셀 기술이 모두 맞을 수 있지만,
  서로 다른 셀의 개수, 좌표, 부피를 섞어서는 안 된다.
- 결정학적 밀도는 모델값이다. 벌크 측정과 비교하려면 대표성 있는 조성,
  상분율, 온도, 설명되지 않은 기공 부피가 없는지가 필요하다.
- 이 글의 3차원 병진 주기성은 관습적인 결정을 다루지만, 현대 결정학의
  정의에는 장거리 질서를 가진 비주기 결정도 포함된다
  ([IUCr, “Crystal”](https://dictionary.iucr.org/Crystal)).

## 관련 노트

- [재료의 원자 구조와 원자 간 결합](atomic-structure-and-interatomic-bonding.md)
- [재료과학 및 공학 프레임워크](materials-science-engineering-framework.md)
- [결정 회절, 역공간, 무질서](crystal-diffraction-reciprocal-space.md)
