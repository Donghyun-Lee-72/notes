...
평형은 측정값이 변하지 않는다는 사실만이 아니라, 주어진 구속 조건에서 허용된
구동 경향이 남아 있지 않은 상태입니다. 거시적 측정값이 일정해도 경계를 통한
전달은 계속될 수 있습니다. 따라서 경계와 허용된 교환을 먼저 명시한 뒤, 허용된
무한소 변화가 계를 더 구동할 수 있는지 판정해야 합니다.
...
(Chang and Oates, 2010, Ch. 1, §1.2, p. 5). 이 조건들은 함께 성립해야 합니다.
온도가 같다는 사실만으로 역학적 또는 화학적 평형이 성립하지는 않습니다.
제0법칙은 온도를 비교할 수 있게 하는 추이적 근거를 제공합니다
...
켤레 힘–변위 항을 더해야 합니다
(Chang and Oates, 2010, Ch. 1, §1.5.2, p. 13). 개방형, 유동형, 구동형,
구배가 있는 계 또는 비평형 정상계에는 닫힌계 최소 규칙을 자동으로 적용할 것이
아니라 수지식과 경계 플럭스가 필요합니다. 따라서 유용한 평형 주장은 계, 허용된
교환, 제어 변수, 상과 조성, 일 방식, 후보 상태가 전역 극값인지 국소 극값인지를
밝혀야 합니다.
